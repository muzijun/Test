...
public class NetManager {
	...
	public void inject(Context context) {
		...
		mNetOperator = new NetOperator(context.getApplicationContext());
	}
	...
	public void fetchAdvertsDetailJsonByRequestParams(String adsID, IResponseListener<JSONObject> listener) {
		...
		if (null != mNetOperator) {
			...
			mNetOperator.fetchJsonByRequestParams(Constant.ADVERTS_DETAIL_URL, Utils.transitionObj2JsonString(map),
					listener);
		} else {
			throw new IllegalArgumentException();
		}
	...
	public void notifyServerWhenInstalled(String adsID, IResponseListener<JSONObject> listener) {
		if (null != mNetOperator) {
			...
			try {
				...
				// String decrypt = new
				// String(RSACodeHelper.decryptByPrivateKey(RSACodeHelper.decode(obj2JsonString)));
	...
	public void fetchApkUrlByAdsID(String adsID, String pakage, IResponseListener<JSONObject> listener) {
		...
		if (null != mNetOperator) {
			...
			mNetOperator.fetchJsonByRequestParams(Constant.FETCH_APK_DOWNLOAD_URL, Utils.transitionObj2JsonString(map),
					listener);
		} else {
			throw new IllegalArgumentException();
		}
	...
	public void fetchTaskTimeByAdsID(String adsID, IResponseListener<JSONObject> listener) {
		if (null != mNetOperator) {
			Map<String, String> map = mFormParams.getBaseParamsMap();
			map.put(Constant.ADVERTS_ID, adsID);
			mNetOperator.fetchJsonByRequestParams(Constant.FETCH_TASK_TIME_URL, Utils.transitionObj2JsonString(map),
					listener);
		} else {
			throw new IllegalArgumentException();
		}
